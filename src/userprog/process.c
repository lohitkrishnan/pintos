...
process_wait (tid_t child_tid UNUSED) 
{
	...
	//check if the given tid is a valid child.
	struct thread *curr = thread_current();
	struct thread *child;
	struct s_child *childs = curr->child_threads;
	int success = 0;
	int i;
	int struct_size = sizeof(struct s_child);
...
load (const char *file_name, void (**eip) (void), void **esp) 
{
	...
	for (i = 0; ; i++, file_name = NULL)
	{
		r=strtok_r(file_name, delimiter, &saveptr);
...
done1:
	...
	//applying the padding of 0's
	for (i = 0; i < roundoff; i++)
	{
		*esp = *esp - 1;
		*((char *)*esp) = 0;
	}
	...
	*((int *)*esp) = 0;
	...
	for (i = end-1 ; i >= 0; i--)
	{
		*esp = *esp - sizeof(arg[i]);
		memcpy(*esp, &arg[i], sizeof(arg[i]));	
	}

	//Address of argv
	last_arg_loc = *esp;
	*esp = *esp - sizeof(arg[0]);
	memcpy(*esp, &last_arg_loc, sizeof(int *));
