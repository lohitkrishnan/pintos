...
thread_init (void) 
{
	...
	list_init(&(initial_thread->retrace_priority_list));
	initial_thread->donated_to = NULL;	
		...
		thread_func *function, void *aux) 
{
	...
	{	
		...
		if((sizeof(struct s_child)* i) >= (PGSIZE - (sizeof(struct s_child))))
		{
			palloc_free_page(t->child_threads);
			palloc_free_page(t);
			return TID_ERROR;
		}
	...
	list_init(&(t->retrace_priority_list));
	/* Add to run queue. */
	thread_unblock (t);
